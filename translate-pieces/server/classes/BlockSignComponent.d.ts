...
export class BlockSignComponent extends BlockComponent {
	...
	/**
	 * @remarks
	 * 表示玩家是否可以编辑告示牌。如果告示牌上使用了蜜脾或调用了 `setWaxed` 方法，则玩家无法编辑。
	 *
	...
	/**
	 * @remarks
	 * 如果使用 `setText` 方法设置了 RawMessage 或 RawText 对象，则返回告示牌的 RawText，否则返回 undefined。
	 *
	...
	/**
	 * @remarks
	 * 如果使用 `setText` 方法设置了字符串，则返回告示牌的文本，否则返回 undefined。
	 *
	...
	/**
	 * @remarks
	 * 获取告示牌文字上的染料颜色，如果未染色则返回 undefined。
	 *
	 * Gets the dye that is on the text or undefined if the sign
	 * has not been dyed.
	 *
	 * @param side
	 * 要读取染料的告示牌面。如果未提供，则返回告示牌正面的染料。
	 *
	 ...
	 *
	 * @param message
	 * 要设置在告示牌上的消息。如果设置为字符串，则可以通过调用 `getText` 来读取该字符串。如果设置为 RawMessage，则调用 `getRawText` 会返回一个 RawText。如果设置为 RawText，则调用 `getRawText` 会返回传入的相同对象。
	 *
